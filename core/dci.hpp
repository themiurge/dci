...
namespace dci
{
    ...
    struct system_properties {
        ...
        inline void init(const size_t& N, const size_t& M, const size_t& NB)
        {
            ...
            bytes_per_sample = S * bytes_per_reg;
            data_bytes = bytes_per_sample * M;
            agent_pool_bytes = bytes_per_sample * N;
        }
    };
    ...
    public:
        ...
        {  
            props.init(N, M, N * bits_per_agent);
            allocate_data();
            ...
            for (size_t a = 0; a != props.N; ++a)
            {
                ...
                reg_type* cur_agent = agent_pool + a * props.S;
                agents[a] = new agent(cur_agent, string(1, '0' + a), this);

                // set bits in agent bitmask
                for (size_t b = 0; b != bits_per_agent; ++b)
                    register_utils::set_bit(cur_agent, b + a * bits_per_agent, 1);
            }
    ...
    istream& operator>>(istream& in, system& s)
    {
        ...
        for (size_t i = 0; i != s.props.M; ++i)
        {
            ...
            register_utils::set_bits_from_string((s.samples[i])->data, lines[i]);
        }
